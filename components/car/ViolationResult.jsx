...
import {
  getAppDataApi,
  getChargeWalletGateWayApi,
  getWalletCreditApi,
} from "@/api/car/wallet_api";
import {
  Table,
  TableHeader,
  TableColumn,
...
const ViolationResult = () => {
  ...
  const [walletCredit, setWalletCredit] = useState(Cookies.get("wallet") || 0);
  ...
  const natCode = Cookies.get("natCode");
  const router = useRouter();
  const REDIRECT_LINK = "https://farmooon.com/";
  const token = Cookies.get("token");
  const [activeModalType, setActiveModalType] = useState(""); // 'image' or 'payment'

  // Fetch wallet credit function
  const fetchWalletCredit = async () => {
    try {
      setShowWalletCreditLoading(true);
      const response = await getWalletCreditApi();
      if (response && response.data) {
        const newWalletCredit = response.data.wallet;
        Cookies.set("wallet", newWalletCredit, {
          secure: false,
          sameSite: "lax",
        });
        setWalletCredit(newWalletCredit);
        console.log("Wallet credit updated:", newWalletCredit);
      }
    } catch (error) {
  ...
  // Detect when user returns from payment gateway using Page Visibility API
  useEffect(() => {
    const handleVisibilityChange = async () => {
      if (document.visibilityState === "visible") {
        // Check if user just returned from payment
        const lastRedirect = localStorage.getItem("lastPaymentRedirect");
        if (lastRedirect && Date.now() - parseInt(lastRedirect) < 120000) {
          // Within 2 minutes
          console.log("User returned from payment gateway, updating wallet...");

          // Wait a moment for backend to process
          setTimeout(async () => {
            await fetchWalletCredit();
            toast.success("پرداخت با موفقیت انجام شد و کیف پول به روز شد");
          }, 1500);

          localStorage.removeItem("lastPaymentRedirect");
        }
      }
    };
  ...
  // Also check URL parameters on component mount as backup
  useEffect(() => {
    const checkUrlParams = async () => {
      ...
      if (paymentStatus === "success" || paymentSuccess === "true" || trackId) {
        console.log("Payment success detected from URL, updating wallet...");
        await fetchWalletCredit();
  ...
  const getPayFineGateWaySubmit = async (billId, payId, index) => {
    const requestData = {
      bills: [
        {
          BillID: billId,
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        // Mark that we're redirecting to payment
        localStorage.setItem("lastPaymentRedirect", Date.now().toString());
  ...
  const getChargeWalletGateWaySubmit = async (amount) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        // Mark that we're redirecting to payment for wallet recharge
        localStorage.setItem("lastPaymentRedirect", Date.now().toString());
  ...
  if (!violationData) {
    return (
      <div className="flex justify-center items-center flex-col mt-10 md:mt-20">
        <p className="font-Kalame text-lg">در حال بارگذاری اطلاعات...</p>
  ...
  return (
    <div className="div-result-container">
      ...
      <p className="font-Kalame font-thin text-base mt-5 mx-auto flex justify-center">
        مجموع مبلغ جرایم : {formatAmount(totalFine)}
      </p>
      ...
      {violationData.bills.map((bill, index) => (
        <div
          className="border border-teal-700 rounded-lg flex mx-auto flex-col w-80 sm:w-96 md:w-1/3 p-4 m-4 text-right"
          key={index}
        >
          <div className="flex justify-around gap-x-24 xl:gap-x-72 items-center border-b-2 w-full pb-2">
            <span className="text-xs font-Kalame">
              مبلغ جریمه :‌ {formatAmount(bill.amount)}
            </span>
            <span className="text-xs font-Kalame">
              {toJalaliDate(bill.violationOccureTime)}
            </span>
          </div>
          ...
          <div className="font-Kalame text-right md:px-4 text-xs mt-2 md:text-sm">
            <span>محل تخلف :‌ {bill.location}</span>
          </div>
          <div className="font-Kalame text-right md:px-4 text-xs mt-2 md:text-sm">
            <span>شرح تخلف :‌ {bill.type}</span>
          </div>
          <div className="font-Kalame text-right md:px-4 text-xs mt-2 md:text-sm">
            <span>شناسه ی قبض :‌ {bill.billId}</span>
          </div>
          ...
          <div className="flex justify-between items-center flex-row mt-5">
            <Button
              ...
              type="submit"
              onClick={() =>
                getPayFineGateWaySubmit(bill.billId, bill.payId, index)
              }
            >
              {payFineLoadingStates[index] ? (
            ...
            {bill.type === "دوربین" ? (
              ...
              >
                {loadingStates[index] ? (
                  ...
                  <span>
                    تصویر {"("} {formatAmount(violationPrice?.image_price) || 0}{" "}
                    تومان {")"}
                  </span>
                )}
              </Button>
            ...
            <>
              ...
              <ModalBody>
                <p className="max-w-screen-lg mx-auto mt-3 font-Kalame text-xs md:text-sm px-4">
                  ✅ تهران – بزرگراه شیخ فضل الله نوری – بلوار مرزداران -شهرک
                  آزمایش پلیس راهنمایی و رانندگی نیروی انتظامی جمهوری اسلامی
                  ایران
                </p>
                  ...
                  <TableBody>
                    ...
                    <TableRow key="3">
                      ...
                      <TableCell className="text-xs md:text-sm">
                        ...
                        آدرس: خیابان شوش نرسیده به میدان شوش پشت بوستان بهاران
                        جنب کلانتری ۱۱۲
                      </TableCell>
                      <TableCell className="text-xs md:text-sm">
                        منطقه ۱۲{" "}
                    ...
                    <TableRow key="4">
                      ...
                      <TableCell className="text-xs md:text-sm">
                        خیابان نبرد ـ میدان نبرد ـ انتهای خیابان شهید ابراهیم
                        بیدی
                      </TableCell>
                      <TableCell className="text-xs md:text-sm">
                        منطقه ۱۴{" "}
                    ...
                    <TableRow key="6">
                      ...
                      <TableCell className="text-xs md:text-sm">
                        بزرگراه نواب ـ تقاطع خیابان محبوب مجاز (ویژه جانبازان ـ
                        ایثارگران و خانواده شهداء){" "}
                      </TableCell>
                      <TableCell className="text-xs md:text-sm">
                        منطقه ۱0{" "}
                      </TableCell>
                    ...
                    <TableRow key="8">
                      ...
                      <TableCell className="text-xs md:text-sm">
                        بزرگراه حقانی ـ نرسیده به تقاطع جهان کودک ـ مقابل پارک
                        آب و آتش{" "}
                      </TableCell>
                      <TableCell className="text-xs md:text-sm">
                        منطقه 3{" "}
      ...
      >
        <ModalContent>
          {(onClose) => (
            <>
              <ModalHeader className="flex flex-col gap-1">
                <h3 className="font-KalameBold text-center">
              ...
              <ModalBody>
                {activeModalType === "image" && imageUrl ? (
                  ...
                  <div>
                    ...
                    <p className="font-Kalame text-center font-semibold my-3">
                      هزینه ی استعلام عکس{" "}
                      {formatAmount(violationPrice?.image_price) || 0} تومان می
                      باشد
                    </p>
                    <Button
                      size="lg"
                    ...
                    >
                      {isChargingWallet ? (
                        ...
                        <span className="text-sm">
                          افزایش اعتبار{" "}
                          {formatAmount(violationPrice?.image_price) || 0} تومان{" "}
                          {"("} یک بار استعلام عکس {")"}
                        </span>
                      )}
                    </Button>
              ...
              <ModalFooter>
                <Button
                  color={activeModalType === "image" ? "primary" : "danger"}
                  variant={activeModalType === "image" ? "shadow" : "light"}
...
export default ViolationResult;
